...
class BybitExchange:
    ...
    async def create_order(self, symbol: str, side: str, order_type: str,
        ...
        if self.mode == 'simulation':
            ...
            # Перевіряємо баланс перед реальною угодою
            balance = await self.get_real_balance('USDT')
            cost = quantity * (price or 0)
            if balance < cost * 1.1:  # 10% запас
    ...
    async def calculate_commission(self, side: str, quantity: float, price: float) -> float:
        return quantity * price * 0.0018

    async def start_websocket(self, symbols: List[str]):
        for symbol in symbols:
            task = asyncio.create_task(self._websocket_worker(symbol))
        ...
        while True:
            try:
                async with websockets.connect(ws_url) as websocket:
                    ...
                    logger.info(f"✅ Підключено WebSocket для {symbol}")
                ...
                logger.error(f"❌ WebSocket помилка {symbol}: {e}")
